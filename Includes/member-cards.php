<!-- Manuel G -->
<div class="member-card" id="member-card-manuel-g">
  <div class="member-card-img-area">
    <img class="member-card-img" src="img/consejo/consejo-manuel-g.png" alt="" />
  </div>
  <div class="member-card-text">
    <span class="member-card-name">Manuel G. Martínez Gaxiola</span>
    <hr/>
    <p>Manuel Martínez es líder en proyectos de transformación de empresas familiares, fusiones y adquisiciones.</p>
    <p>El Sr. Martínez es Licenciado y Maestro de Administracion con especialidad en Análisis Estratégico por el ITESM. Egresado del programa de Formación de Consejeros en Administración por la EGADE del ITESM, del Programa de Alta Dirección (AD-2) y del programa de Alta Dirección en Capital Privado por el IPADE.</p>
    <a class="member-card-button" href="/vcards/Manuel_G._Martinez_Gaxiola.vcf">Descargar vCard</a>
  </div>
</div>

<!-- Carlos J -->
<div class="member-card" id="member-card-carlos-j">
  <div class="member-card-img-area">
    <img class="member-card-img" src="img/consejo/consejo-carlos-m.png" alt="" />
  </div>
  <div class="member-card-text">
    <span class="member-card-name">Carlos J. Martínez de León</span>
    <hr/>
    <p>Carlos Martínez cuenta con amplia experiencia en la creación y desarrollo de empresas familiares, diseño de alianzas estratégicas e implementación de metodologías para alinear la organización hacia los objetivos estratégicos.</p>
    <p>El Sr. Martínez es un Ingeniero Industrial y de Sistemas por el ITESM y Maestro en Business Administration por la Universidad de Texas. Está certificado en Alta Dirección de Capital Privado por el IPADE y como Latin Amerian Private Equity Professional por la Wharton School de la Universidad de Pennsylvania. Además, es egresado del Family Wealth Management Program por la Universidad de Columbia.</p>
    <a class="member-card-button" href="/vcards/Carlos_Martinez_de_Leon.vcf">Descargar vCard</a>
  </div>
</div>

<!-- Oscar J -->
<div class="member-card" id="member-card-oscar-j">
  <div class="member-card-img-area">
    <img class="member-card-img" src="img/consejo/consejo-oscar-j.png" alt="" />
  </div>
  <div class="member-card-text">
    <span class="member-card-name">Oscar J. Moráles Rodríguez</span>
    <hr/>
    <p>Oscar Morales es especialista en banca privada en una banca con presencia global. Cuenta con amplio conocimiento en implementación de asesoría y manejo de portafolios de inversión con clientes de alto patrimonio.</p>
    <p>El Sr. Morales es Licenciado en Administracion de Empresas por el ITESM y está certificado en Estrategias de Inversion por la Asociación Mexicana de Intermediarios Bursátiles para asesorar a clientes en portafolios de inversión. Es egresado del Programa de Alta Dirección de Capital Privado impartido por el IPADE y actualmente cursa la Maestría en Administración de Empresas por la EGADE del ITESM.</p>
    <a class="member-card-button">Descargar vCard</a>
  </div>
</div>

<!-- Carlos E -->
<div class="member-card" id="member-card-carlos-e">
  <div class="member-card-img-area">
    <img class="member-card-img" src="img/consejo/consejo-carlos-e.png" alt="" />
  </div>
  <div class="member-card-text">
    <span class="member-card-name">Carlos E. Martínez Rico</span>
    <hr/>
    <p>Carlos Martínez Rico forma parte de despachos jurídicos de presencia y prestigio internacional
      y cuenta con amplia experiencia en asesoramiento e implementación de fusiones y adquisiciones,
      así como en la negociación de contratos.</p>
    <p>El Sr. Martínez es Licenciado en Derecho por la Escuela Libre de Derecho. Cuenta con un Posgrado
      en Derecho Fiscal por la Universidad Panamericana, así como con una especialidad en la misma materia por la UNAM.
      Asimismo, obtuvoel grado de Maestría en Derecho por la Universidad de Georgetown.
    </p>
    <a class="member-card-button">Descargar vCard</a>
  </div>
</div>

<!-- Lorenzo F -->
<div class="member-card" id="member-card-lorenzo-f">
  <div class="member-card-img-area">
    <img class="member-card-img" src="img/consejo/consejo-lorenzo-f.png" alt="" />
  </div>
  <div class="member-card-text">
    <span class="member-card-name">Lorenzo Fernández</span>
    <hr/>
    <p>Lorenzo Fernández es miembro del Consejo y del Comité de Dirección, así como Director de sede de una de las
      escuelas de negocios más importantes del mundo. Cuenta con amplia experiencia directiva en valuación, análisis financiero
      e implementación de conceptos de creación de valor, asociación y ventas de empresas, así como en consejos de administración.</p>
    <p>El Sr. Fernández realizó estudios profesionales de Ingeniería Industrial en la Universidad Panamericana. Cuenta con un
    Posgrado en Dirección de Empresas por el IPADE y cursó el diplomado de The United States Mergers and Acquisitions impartido
  en New York Institute of Finance. Asimismo, ha sido profesor de finanzas en escuelas de negocios en Colombia, Argentina, Brasil,
Guatemala, Perú y México</p>
    <a class="member-card-button">Descargar vCard</a>
  </div>
</div>
